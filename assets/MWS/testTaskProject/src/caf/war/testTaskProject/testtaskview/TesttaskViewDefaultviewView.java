 ...
 * 
 */
package caf.war.testTaskProject.testtaskview;
...
@ManagedBean(name = "TesttaskViewDefaultviewView")
@SessionScoped
@ExpireWithPageFlow
@DTManagedBean(displayName = "TesttaskView/default", beanType = BeanType.PAGE)
public class TesttaskViewDefaultviewView extends com.webmethods.caf.faces.bean.task.BaseTaskDetailsPageBean {
	...
	private static final String[][] TASKDISPLAYPROVIDER_PROPERTY_BINDINGS = new String[][] {
		{"#{TaskDisplayProvider.taskID}", "#{TesttaskView.taskID}"},
	};
	...
	public String cancelView() {
		try {
			...
			String url = getTesttaskView().getFinishUrl();
	...
	public String assignToPrincipals() {
		try {
			...
			TaskContentProvider tp = getTesttask();
	...
	private transient caf.war.testTaskProject.testtaskview.TesttaskView testtaskView = null;
	...
	private static final String[][] TESTTASK_PROPERTY_BINDINGS = new String[][] {
		{"#{Testtask.taskID}", "#{TesttaskView.taskID}"},
		{"#{Testtask.autoAccept}", "false"},
		{"#{Testtask.adhocRouting}", "false"},
	};
	...
	public caf.war.testTaskProject.testtaskview.TesttaskView getTesttaskView()  {
		if (testtaskView == null) {
		    testtaskView = (caf.war.testTaskProject.testtaskview.TesttaskView)resolveExpression("#{TesttaskView}");
		}
		return testtaskView;
	}
